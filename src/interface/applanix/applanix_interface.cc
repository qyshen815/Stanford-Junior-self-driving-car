...
{
  callbacks->AddCallback("APPLANIX_POSE_V1", ApplanixPoseID,
			 (LogConverterFunc)StringV1ToApplanixPose, 
			 sizeof(ApplanixPose), 0);
  callbacks->AddCallback("APPLANIX_POSE_V2", ApplanixPoseID,
			 (LogConverterFunc)StringV2ToApplanixPose, 
			 sizeof(ApplanixPose), 0);
  callbacks->AddCallback("APPLANIX_RMS_V1", ApplanixRmsID,
			 (LogConverterFunc)StringToApplanixRms, 
			 sizeof(ApplanixRms), 0);
  callbacks->AddCallback("APPLANIX_GPS_V1", ApplanixGpsID,
			 (LogConverterFunc)StringToApplanixGps, 
			 sizeof(ApplanixGps), 0);
  callbacks->AddCallback("APPLANIX_DMI_V1", ApplanixDmiID,
			 (LogConverterFunc)StringToApplanixDmi,
			 sizeof(ApplanixDmi), 0);
}
...
{
  dgc_fprintf(outfile, 
	     "APPLANIX_POSE_V2 %.9f %.9f %f %.9f %.9f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %u %d %f %d %f %s %f\n",
	     pose->smooth_x, pose->smooth_y, pose->smooth_z,      
             pose->latitude, pose->longitude, pose->altitude, pose->v_north,
             pose->v_east, pose->v_up, pose->speed, pose->track, pose->roll,
             pose->pitch, pose->yaw, pose->ar_roll, pose->ar_pitch,
             pose->ar_yaw, pose->a_x, pose->a_y, pose->a_z, pose->wander, 
             pose->ID, pose->postprocess_code, pose->hardware_timestamp, 
             pose->hardware_time_mode, pose->timestamp, pose->host,
             logger_timestamp);
}
...
{
  dgc_fprintf(outfile, 
             "APPLANIX_RMS_V1 %f %f %f %f %f %f %f %f %f %f %f %f %u %d %f %d %f %s %f\n",
             rms->rms_north, rms->rms_east, rms->rms_up, rms->rms_v_north, 
             rms->rms_v_east, rms->rms_v_up, rms->rms_roll, rms->rms_pitch, 
             rms->rms_yaw, rms->semi_major, rms->semi_minor, 
             rms->orientation, rms->ID, rms->postprocess_code, rms->hardware_timestamp, rms->hardware_time_mode,
             rms->timestamp, rms->host, logger_timestamp);
}

void ApplanixGpsWrite(ApplanixGps *gps, double logger_timestamp, 
		      dgc_FILE *outfile)
{
  dgc_fprintf(outfile, "APPLANIX_GPS_V1 %d %u %f %d %u %f %d %u %f %f %s %f\n",
             gps->primary_sats, gps->primary_ID, gps->primary_timestamp,
             gps->secondary_sats, gps->secondary_ID, gps->secondary_timestamp,
             gps->gams_solution_code, gps->gams_ID, gps->gams_timestamp,
             gps->timestamp, gps->host, logger_timestamp);
}

void ApplanixDmiWrite(ApplanixDmi *dmi, double logger_timestamp, 
		      dgc_FILE *outfile)
{
  dgc_fprintf(outfile, "APPLANIX_DMI_V1 %f %f %u %f %d %f %s %f\n",
	      dmi->signed_odometer, dmi->unsigned_odometer, dmi->ID, 
	      dmi->hardware_timestamp, dmi->hardware_time_mode, 
	      dmi->timestamp, dmi->host, logger_timestamp);
}
...
{
  ipc->AddLogHandler(ApplanixPoseID, NULL, sizeof(ApplanixPose),
		     (dgc_log_handler_t)ApplanixPoseWrite,
		     start_time, logfile, subscribe_how);
  ipc->AddLogHandler(ApplanixRmsID, NULL, sizeof(ApplanixRms),
		     (dgc_log_handler_t)ApplanixRmsWrite,
		     start_time, logfile, subscribe_how);
  ipc->AddLogHandler(ApplanixGpsID, NULL, sizeof(ApplanixGps),
		     (dgc_log_handler_t)ApplanixGpsWrite,
		     start_time, logfile, subscribe_how);
  ipc->AddLogHandler(ApplanixDmiID, NULL, sizeof(ApplanixDmi),
		     (dgc_log_handler_t)ApplanixDmiWrite,
		     start_time, logfile, subscribe_how);
}
