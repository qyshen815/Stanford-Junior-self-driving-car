...
TrackManager::TrackManager(const std::vector< shared_ptr<Track> >& tracks)  :
  serialization_version_(TRACKMANAGER_SERIALIZATION_VERSION),
  tracks_(tracks)
{
}
...
double getTrackLength(const Track& tr) {
  double z = tr.getMeanNumPoints() + tr.frames_[0]->cloud_->get_points_size() + tr.frames_[0]->robot_pose_.roll + tr.frames_[0]->robot_pose_.pitch + tr.frames_[0]->robot_pose_.yaw;
...
bool Track::deserialize(istream& istrm) {
  ...
  for(size_t i=0; i<num_frames; ++i) {
    assert(!frames_[i]);
    frames_[i] = shared_ptr<Frame>(new Frame(istrm, velodyne_offset_));
  }
...
double Track::getMeanNumPoints() const {
  double total = 0;
  for(size_t i = 0; i < frames_.size(); ++i) {
    total += frames_[i]->cloud_->get_points_size();
  }
...
void Track::setVelodyneOffset(const dgc_transform_t& velodyne_offset) {
  for(int i = 0; i < 4; ++i)
    for(int j = 0; j < 4; ++j)
...
Track::Track(const std::string& label,
			    ...
			    const std::vector< shared_ptr<Frame> >& frames) :
...
{
  frames_.push_back(shared_ptr<Frame>(new Frame(cloud, timestamp, robot_pose, velodyne_offset_)));
}
...
Frame::Frame(istream& istrm, dgc_transform_t velodyne_offset) :
  spin_offset_(-1),
  serialization_version_(FRAME_SERIALIZATION_VERSION)
{
...
bool Frame::getCloudInVeloCoords(Eigen::MatrixXd* eig) const {
  if(cloud_->get_points_size() < 1)
    return false;
    
  *eig = MatrixXd(cloud_->get_points_size(), 4);
...
Matrix4f Frame::getSmoothToVeloTransform(dgc_transform_t velodyne_offset) const {
  ...
  dgc_transform_t inv;
  dgc_transform_rpy(original, velodyne_offset, robot_pose_.roll, robot_pose_.pitch, robot_pose_.yaw);
...
void Frame::getVelodyneXYZ(dgc_transform_t velodyne_offset, double* x, double* y, double* z) const {
  ...
  *z = laser_z;
  ...
  Vector3f Frame::getCentroid() {
    ...
    centroid_ = shared_ptr<Vector3f>(new Vector3f());
    ...
    *centroid_ /= (double)cloud_->get_points_size();
  ...
  MatrixXf Frame::getBoundingBox() {
    ...
    bounding_box_ = shared_ptr<MatrixXf>(new MatrixXf(2, 2));
  ...
  double Frame::getDistance(dgc_transform_t velodyne_offset) {
  ...
  double Frame::estimateAdjustedTimestamp() {
    ...
    double adjusted_timestamp = timestamp_ + estimateSpinOffset() * spin_time;
...
bool cloudsEqual(const PointCloud& c1, const PointCloud& c2) {
...
shared_ptr<PointCloud> getRandomCloud() {
  int num_pts = 100;
...
shared_ptr<Frame> getRandomFrame(dgc_transform_t velodyne_offset) {
  shared_ptr<PointCloud> cloud = getRandomCloud();
  ...
  return shared_ptr<Frame>(new Frame(cloud, timestamp, robot_pose, velodyne_offset));
}

shared_ptr<Track> getRandomTrack() {
  int num_frames = 10;
  ...
  vector< shared_ptr<Frame> > frames(num_frames);
  for(size_t i = 0; i < frames.size(); ++i)
    frames[i] = getRandomFrame(velodyne_offset);

  return shared_ptr<Track>(new Track(label, velodyne_offset, frames));
}

shared_ptr<TrackManager> getRandomTrackManager() {
  int num_tracks = 15;

  vector< shared_ptr<Track> > tracks(num_tracks);
  for(size_t i = 0; i < tracks.size(); ++i)
    tracks[i] = getRandomTrack();

  return shared_ptr<TrackManager>(new TrackManager(tracks));
}
