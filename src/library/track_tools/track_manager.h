...
namespace track_manager { 
  ...
  public:
    ...
    Frame(std::istream& istrm, dgc_transform_t velodyne_offset);
    void serialize(std::ostream& out) const;
    bool deserialize(std::istream& istrm);
    Eigen::Matrix4f getSmoothToVeloTransform(dgc_transform_t velodyne_offset) const;
    ...
    bool operator==(const Frame& fr);
    void getVelodyneXYZ(dgc_transform_t velodyne_offset, double* x, double* y, double* z) const;
    Eigen::Vector3f getCentroid();
    Eigen::MatrixXf getBoundingBox();
    double getDistance(dgc_transform_t velodyne_offset);
  ...
  public:
    ...
    void setVelodyneOffset(const dgc_transform_t& velodyne_offset);
