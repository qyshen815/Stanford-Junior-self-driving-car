...
protected:
	typedef StBase kogmo_base;
	bool isExpired(const Timestamp& t) {
		Timestamp now;
		now.now();
		return now > t;
	}
	...
	bool detectedErrornousTransitions() {
		...
		//std::cout << "HELLLLLLLLO OO OOOOOOOOO: " << planner.transitionCounter << std::endl;
		planner.transitionCounter = planner.transitionCounter+1;
		if (planner.transitionCounter > 16) { // doesn't work, don"t know why
			planner.addMessage("errournous transitions detected");
			return true;
		} else {
			return false;
		}
  ...
  Timestamp recovery_timeout;
  Pose recovery_last_pose_;
