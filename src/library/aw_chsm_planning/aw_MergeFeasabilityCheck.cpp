...
MergeFeasabilityCheck::Entities MergeFeasabilityCheck::getEntities(RndfEdge* edge, double offset, double dist_in_from_direction, double dist_in_to_direction, bool same_lane)
{
	...
	stack.push_back(StackEntry(edge, edge->getLength() - offset));
//	TRACE("++++++++++++++++++");
	while (stack.size()) {
...
//				TRACE("   found vehicle (not added) "<<iter->second);
		...
		d -= current.edge->getLength();
		if (d > -dist_in_from_direction) {
//			TRACE("  fromvertex " << current.edge->fromVertex()->name() << " size="<< current.edge->fromVertex()->getInEdges().size());
			const set< RndfEdge* >& edges = current.edge->fromVertex()->getInEdges();
			for (set< RndfEdge* >::const_iterator iter = edges.begin();	iter != edges.end(); ++iter) {
...
//			TRACE("  tovertex " << current.edge->toVertex()->name() << " size="<< current.edge->toVertex()->getOutEdges().size());
			for (RndfVertex::TEdgeSet::const_iterator iter = current.edge->toVertex()->getOutEdges().begin(); iter != current.edge->toVertex()->getOutEdges().end(); ++iter) {
				assert(*iter != current.edge);
				assert(*iter);
				stack.push_back(StackEntry(*iter, d));
//				TRACE("  adding "<<(*iter)->name() << " d="<<d);
