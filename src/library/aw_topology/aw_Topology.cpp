...
Topology::Topology(const char* rndf_filename, const char* mdf_filename, std::string name, bool recover_from_file_, bool save_mission_progress_to_file_):
  complete_graph(NULL),
  ego_vehicle(),
  route(),
  ...
  save_mission_progress_to_file( save_mission_progress_to_file_ ),
  reached_cp_count( 0 )
{
...
void Topology::plan_mission(double initial_x, double initial_y, double initial_yaw) {
  mission_planned = false;

  //  bool firstPoint = true;
  CheckpointList* cpl = m.getCheckpointList();
  if(!cpl) {
      throw Exception("Failed to plan mission: check point list is invalid (empty?)");
  }
  ...
  if (recover_from_file) {
    ...
    for (int i = 0; i < reached_cp_count; i++) {
      cpl->getCheckpoints().pop_front();
      if (cpl->getCheckpoints().size() == 1) {
        reached_cp_count--;
  ...
  assert(cpl->size()>0);
  if(cpl->empty()) {
      throw Exception("Failed to plan mission: check point list is empty");
  }
  ...
  { // scope for auto_ptr
    ...
    while (!found_valid_mission_start) {
      ...
      RndfVertex* v1 = complete_graph->getCheckpoint(lexical_cast<int>((*cpl->begin())->name()));
      if (cpl->size()>1) {
        ...
        RndfVertex* v2 = complete_graph->getCheckpoint(lexical_cast<int>((*(cpl->begin()+1))->name()));
            ...
            for (std::set<RndfEdge*>::const_iterator iter = possibleStarts.begin(); iter != possibleStarts.end(); ++iter) {
              ...
              if (!visited_vertexes[(*iter)->toVertex()]) {
                ...
                if (&*edges != 0) { // found a way
                  ...
                  cost += (*iter)->getLength(); // we want to priorise fromVertexes
  ...
  for (TCheckpointVec::const_iterator it=cpl->begin(); it != cpl->end(); ++it, ++i) {
    ...
    missionPoints.push_back(complete_graph->getCheckpoint(lexical_cast<int>((*it)->name())));
  }
...
//      cout << "planning " << start << "->" << goal << "... " << flush;
  ...
  // update ego pose
  update_ego_vehicle(initial_x, initial_y, initial_yaw, 0);

  // set checkpoint list
  checkpoints.clear();
  const rndf::TCheckpointVec& rndf_cps = m.getCheckpointList()->getCheckpoints();
  ...
  old_checkpoint_iter = getNextCheckpointIter();
  //route.dump();
...
//  		std::cout << *(iter->second) << " length="<<iter->second->getLength() << std::endl;
//  	}
//  }
  //::exit(1);
}

void Topology::update_ego_vehicle( const double x, const double y, const double yaw, const double speed )
{
...
Vehicle* Topology::next_obstacle( double& distance, GraphTools::PlaceOnGraph& place, double max_scan_distance  )
{
...
//  TRACE("Rematch EgoVehicle: "<< ego_vehicle.edge->name());
  ...
  for( current_edge_it = route.route.begin(); current_edge_it != route.route.end(); current_edge_it++ )
  {
    ...
    if( (*current_edge_it)->getEdge() == ego_vehicle.edge() )
    {
  ...
  if( mission_planned && !(checkpoints.size() == 0) )
      ...
      if( !(*current_edge_it)->was_reached )
	{
	  ...
	  if( save_mission_progress_to_file )
	    {
	      std::cout << "last cp " << (*getNextCheckpointIter())->checkpointId() << " next cp " << (*old_checkpoint_iter)->checkpointId() << "\n";
	      bool count_it = false;
	      int id;
	      if( (*getNextCheckpointIter())->checkpointId() != (*old_checkpoint_iter)->checkpointId() )
		{
		  count_it = true;
	      ...
	      if( (*current_edge_it)->getEdge()->isZoneEdge() )
	      {
	      if( (*current_edge_it)->isToCheckpointEdge() &&
	      (*current_edge_it)->getEdge()->toVertex()->isCheckpointVertex() &&
	      (*current_edge_it)->getEdge()->toVertex()->checkpointId() != -1 )
	      {
	      id = (*current_edge_it)->getEdge()->toVertex()->checkpointId();
	      count_it = true;
	      }
	      }
	      else
	      ...
	      (*current_edge_it)->getEdge()->fromVertex()->isCheckpointVertex() &&
	      (*current_edge_it)->getEdge()->fromVertex()->checkpointId() != -1 )
	      {
	      id = (*current_edge_it)->getEdge()->fromVertex()->checkpointId();
	      count_it = true;
	      }
  ...
  while( ( count > 2 ) && ( passed_distance > BACK_VIEW_DISTANCE ) )
  {
    ...
    passed_distance -= route.route.front()->getEdge()->getLength();
  ...
  /*std::cout << "blub dist_to_next_parking_spot = " << dist_to_next_parking_spot()
    << " dist_to_next_zone = " << dist_to_next_zone()
    << " dist_to_next_zone_exit = " << dist_to_next_zone_exit()
    << std::endl;*/
}
...
void Topology::extract_relevant_part_of_route()
{
  ...
  for( Route::RouteEdgeList::const_iterator it = route.route.begin(); it != route.route.end(); it++ )
  {
    ...
    preview_len += (*it)->getEdge()->getLength();

    if( c>2 && ((*it)->getEdge()->visited || ( preview_len > ( MIN_PREVIEW_LEN + BACK_VIEW_DISTANCE  ) )) )
    {
      break;
      //hit_older_part_of_mission = true;
    }
    mission_graph_map[idx] = (*it)->getEdge();
    (*it)->getEdge()->visited = true;
    idx++;
  }
  ...
  for( Route::RouteEdgeList::const_iterator it = route.route.begin(); it != route.route.end(); it++ )
  {
    (*it)->getEdge()->visited = false;
  }
}

double Topology::dist_to_next_maneuver(maneuver_t maneuver)
{
...
double Topology::dist_to_next_maneuver_start(maneuver_t maneuver)
{
  if(!mission_planned) return std::numeric_limits<double>::infinity();
  ...
  RoutePlanner::AnnotatedRouteEdge* edge = place.next_manuever( d, place, TRIGGER_DIST_MAX_LOOKAHEAD, maneuver );
  if (edge) {
    return d - FRONT_BUMPER_DELTA - edge->getEdge()->getLength();
  }
  return std::numeric_limits<double>::infinity();
}


double Topology::dist_to_next_area(area_type_t area)
{
  double d;
...
RndfIntersection* Topology::get_next_intersection()
{
  ...
  if (redge && redge->getEdge()) return redge->getEdge()->getIntersection();
  return NULL;
}

double Topology::dist_to_next_intersection()
{
  double dist1 = dist_to_next_maneuver(UC_MANEUVER_INT_TURN_RIGHT);
  double dist2 = dist_to_next_maneuver(UC_MANEUVER_INT_TURN_LEFT);
  double dist3 = dist_to_next_maneuver(UC_MANEUVER_INT_STRAIGHT);
  dist1 = min(dist1, dist2);
  dist1 = min(dist1, dist3);
  return dist1;
}

double Topology::dist_to_next_lanechange()
{
    if(!mission_planned) return numeric_limits<double>::infinity();

    double dist_1 = dist_to_next_maneuver_start(UC_MANEUVER_LANECHANGE_LEFT);
    double dist_2 = dist_to_next_maneuver_start(UC_MANEUVER_LANECHANGE_RIGHT);

    return min(dist_1, dist_2);
}

double Topology::dist_to_next_crosswalk() {
    return dist_to_next_crosswalk(NULL, NULL);
}

double Topology::dist_to_next_crosswalk(std::vector<std::string>* cw_names, CurvePoint* cw_pos)
{
  if(!mission_planned) return std::numeric_limits<double>::infinity();
  ...
  if(edge->getEdge()->toVertex()->isExitVertex()) {
    return std::numeric_limits<double>::infinity();
  }
...
//  RoutePlanner::AnnotatedRouteEdge* edge = place.go_bwd_to_crosswalk_light(bwd_dist, TRIGGER_DIST_MAX_LOOKAHEAD, isec );
    ...
    if(cw_names) {
      const std::set<std::string>& cw_name_set = edge->getEdge()->toVertex()->crosswalkNames();
    ...
    if(cw_pos) {
      cw_pos->x = edge->getEdge()->toVertex()->x();
      cw_pos->y = edge->getEdge()->toVertex()->y();
     }
...
double Topology::dist_to_next_traffic_light() {
    return dist_to_next_traffic_light(NULL, NULL);
}

double Topology::dist_to_next_traffic_light(std::vector<std::string>* tl_names, CurvePoint* tl_pos)
{
  if(!mission_planned) return std::numeric_limits<double>::infinity();
...
//  RoutePlanner::AnnotatedRouteEdge* edge = place.go_bwd_to_traffic_light(bwd_dist, TRIGGER_DIST_MAX_LOOKAHEAD, isec );
    ...
    if(tl_names) {
      const std::set<std::string>& cw_name_set = edge->getEdge()->toVertex()->trafficLightNames();
      std::set<std::string>::const_iterator cwit=cw_name_set.begin(), cwit_end=cw_name_set.end();
      for(;cwit != cwit_end; cwit++) {
    ...
    if(tl_pos) {
      tl_pos->x = edge->getEdge()->toVertex()->x();
      tl_pos->y = edge->getEdge()->toVertex()->y();
     }
  //  return (fwd_dist <= bwd_dist ? fwd_dist : - bwd_dist);
  return fwd_dist;
}
...
double Topology::dist_to_next_stopline()
{
  double d;
  ...
  double d_sum = 0;
  while (maneuver != NULL && !maneuver->getEdge()->toVertex()->hasIntersectionOutEdge()) {
    d_sum += d;
    place = next_place;
...
double Topology::dist_to_next_sole_stopline(CurvePoint& stop_point)
{
  double d;
  ...
  if(maneuver) {
    stop_point.x = (*next_place.edge)->getEdge()->toVertex()->x();
    stop_point.y = (*next_place.edge)->getEdge()->toVertex()->y();
    return d + d_sum - FRONT_BUMPER_DELTA;
  }
  return std::numeric_limits<double>::infinity();
}

double Topology::dist_to_next_kturn()
{
  return dist_to_next_maneuver(UC_MANEUVER_U_TURN);
}

double Topology::dist_to_next_zone()
{
  return std::max(0.0,std::min(dist_to_next_area(UC_ZONE), dist_to_next_area(UC_PARKING_ZONE)));
}
double Topology::dist_to_next_zone_exit()
{
  double d;
...
double Topology::dist_to_next_parking_spot()
{
  return dist_to_next_maneuver(UC_MANEUVER_PARKING);
}

double Topology::dist_to_next_moving_veh()
{
  double d;
...
double Topology::dist_to_next_standing_veh()
{
  double d;
...
double Topology::dist_to_prev_veh()
{
  double d;
...
double Topology::speed_of_next_veh()
{
  double d;

  if(!mission_planned) return 0.0;
...
double Topology::dist_to_mission_end() {
  ...
  RndfEdge* edge = (*curr_edge_it)->getEdge();
  assert(edge);
...
double Topology::dist_to_intersection(RndfIntersection* isec)
{
  if(!mission_planned) return std::numeric_limits<double>::infinity();
...
double Topology::dist_to_stopline(RndfIntersection* isec) {
  return dist_to_stopline(isec, NULL);
}

double Topology::dist_to_stopline(RndfIntersection* isec, CurvePoint* cp)
{
  if(!mission_planned) return std::numeric_limits<double>::infinity();
...
//  return (fwd_dist <= bwd_dist ? fwd_dist : - bwd_dist);
  ...
  if(cp && edge) {
    cp->x = edge->getEdge()->toVertex()->x();
    cp->y = edge->getEdge()->toVertex()->y();
  }
  return fwd_dist;
}

double Topology::dist_to_traffic_light(RndfIntersection* isec) {
  return dist_to_traffic_light(isec, NULL, NULL);
}

double Topology::dist_to_traffic_light(RndfIntersection* isec, std::vector<std::string>* tl_names, CurvePoint* tl_pos)
{
  if(!mission_planned) return std::numeric_limits<double>::infinity();
...
//  RoutePlanner::AnnotatedRouteEdge* edge = place.go_bwd_to_traffic_light(bwd_dist, TRIGGER_DIST_MAX_LOOKAHEAD, isec );
  ...
  if(tl_names) {
    const std::set<std::string>& tl_name_map = edge->getEdge()->toVertex()->trafficLightNames();
...
int Topology::nextTurnDirection()
{
  ...
  while (dist < TRIGGER_DIST_MAX_LOOKAHEAD) {
  	// Check Annotations
    AnnotatedRouteEdge::AnnotationList annos = (*curr_edge_it)->getAnnotations();
    for (AnnotatedRouteEdge::AnnotationList::iterator it=annos.begin(); it != annos.end(); ++it) {
      switch ((*it)->maneuver()) {
      case UC_MANEUVER_INT_TURN_RIGHT : return TURN_SIGNAL_RIGHT;
      case UC_MANEUVER_INT_TURN_LEFT  : return TURN_SIGNAL_LEFT;
      case UC_MANEUVER_INT_STRAIGHT : return TURN_SIGNAL_NONE;
      default: break;
      }
    }
    // Eine Kante weiter gehen
...
bool Topology::isRouteBlocked(double max_scan_dist, bool stop_scan_at_checkpoint)
{
	...
	while ( edge_it != route.route.end() && dist < max_scan_dist ) {
		...
		if ( stop_scan_at_checkpoint && (*edge_it)->getEdge()->toVertex() == (*next_check_point_it) ) return false;
		dist += edge->getLength();
		++edge_it;
	}
	return false;
...
deque< RndfVertex* >::iterator Topology::getNextCheckpointIter()
{
  if(checkpoints.empty()) {
    throw vlr::Exception("Checkpointlist is empty.");
  }

  if(complete_mission_graph.empty()) {
    throw vlr::Exception("(Complete) mission graph is empty.");
  }
...
//		TRACE("  "<< i << ".  "<< (*it)->name() );
  ...
  if(current_edge_it_on_complete_mission_graph == complete_mission_graph.end()) {
    throw vlr::Exception("Current edge (matched to mission graph) is not on mission graph.");
  }
	...
	for (RoutePlanner::Route::RouteEdgeList::iterator it = complete_mission_graph.begin(); it != complete_mission_graph.end(); ++it, ++i) {
		...
		if( (*it)->getEdge()->isZoneEdge() )
		  {
		    if ( (*it)->getEdge()->toVertex() == *cp_it && cp_it == checkpoints.begin() )
		      ++cp_it;
		  }
		else
		  {
		    if ( (*it)->getEdge()->fromVertex() == *cp_it && cp_it == checkpoints.begin() )
		      ++cp_it;
		  }

		if ( *it == *edge_it ) break;
		if ( (*it)->getEdge()->toVertex() == *cp_it )
			++cp_it;
...
RndfEdge* Topology::getBestReplanningEdge()
{
	...
	for (BlockadeManager::TBlockageMap::const_iterator it = blockade_manager->blockade_map.begin(); it != blockade_manager->blockade_map.end(); ++it) {
		...
		if ( block.isPublished() && ego_vehicle.distToEdge( block.getEdge() ) < 50.) {
			RndfEdge* edge = block.getEdge();
	...
	for (map<double, RndfEdge*>::const_iterator match_it = base_matchings.begin(); match_it != base_matchings.end(); ++match_it)
	{
		...
		for (vector< Point_2 >::iterator it = blocks.begin(); it != blocks.end(); ++it) {
			...
			if ( squared_distance( block_point, edge->getSegment() ) < sqr( 7. ) ) {
				skip = true;
				break;
			}
			...
			if ( ( line.has_on_negative_side( edge->fromVertexPoint() ) || line.has_on_negative_side( edge->toVertexPoint() ) ) &&
					squared_distance( block_point, edge->getSegment() ) < sqr( 50. ) )
			{
	...
	for (map<double, RndfEdge*>::const_iterator match_it = matching.begin(); match_it != matching.end(); ++match_it)
	{
		...
		if(edge->fromVertex()->getInEdges().size()==1) {
		  if((*edge->fromVertex()->getInEdges().begin())->isUTurnEdge()) {
		...
		if ( edge->isUTurnEdge() ||
			...
			(*edge->fromVertex()->getInEdges().begin())->isUTurnEdge()) ) {
			TRACE("  -> UTurnEdge (skip)");
			continue;
		}
...
bool Topology::isOffTrack()
{
	...
	double dist_to_edge = ego_vehicle.distToMatchedEdge() - ego_vehicle.edge()->getWidth();
...
double Topology::calc_lateral_offset(Vehicle*)// vehicle)
{
	assert(false);
	return 4.5;

//	double dirx, diry, nx, ny, nlen;
//  RndfVertex* fromVertex, *toVertex;
//
//  double offset=vehicle->distFromStart();
//  fromVertex=vehicle->edge->fromVertex();
//  toVertex=vehicle->edge->toVertex();
//
//  dirx=toVertex->x()-fromVertex->x();
//  diry=toVertex->y()-fromVertex->y();
//  nlen=sqrt(dirx*dirx+diry*diry);
//  if (nlen < 0.01) {
//    return MIN_PASSING_LATERAL_OFFSET;
//  }
//  nx=-diry/nlen;
//  ny=dirx/nlen;
//  double d=nx*(vehicle->xMatchedFrom()-vehicle->x_matched)+ny*(vehicle->yMatchedFrom()-vehicle->y_matched);
//  offset = max( vehicle->width + d + SAFETY_DISTANCE, MIN_PASSING_LATERAL_OFFSET) ;
//  //  printf("vehicle width: %f, distance to graph: %f => offset %f\n", vehicle->width, d, offset);
//
//  return offset;
}
...
bool Topology::is_left_turn_edge(const RndfEdge* edge) const {
  double angle_a = atan2( edge->fromVertex()->y() - edge->toVertex()->y(), edge->fromVertex()->x() - edge->toVertex()->x() );
  while( angle_a < -M_PI ) {angle_a += M_PI;}
  while( angle_a >= M_PI ) {angle_a -= M_PI;}

  const RndfVertex* to_vertex = edge->toVertex();
  if( to_vertex->numOutEdges() != 1 ) return false;

  RndfEdge* next_edge = *(to_vertex->beginEdges());

  double angle_b = atan2( next_edge->fromVertex()->y() - next_edge->toVertex()->y(), next_edge->fromVertex()->x() - next_edge->toVertex()->x() );
  while( angle_b < -M_PI ) angle_b += M_PI;
  while( angle_b >= M_PI ) angle_b -= M_PI;

  if( ( angle_b - angle_a ) < -0.0001 ) {return false;}
  return true;
}
...
void Topology::paint_annotations( double center_x, double center_y, AnnotatedRouteEdge* annoedge, double height)
{
  ...
  double b=0.711;
  bool override = false;
  for( ait=annoedge->getAnnotations().begin(); ait!=annoedge->getAnnotations().end(); ++ait )
  {
...
//    }
  ...
  draw_edge(center_x, center_y, *(annoedge->getEdge()), r/255.0, g/255.0, b/255.0, height );
}
