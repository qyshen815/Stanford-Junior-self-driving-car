...
std::map< RoutePlanner::RndfEdge*, double > multi_match_2_graph( const Vehicle& veh, const set< RndfEdge* >& edges )
{
	...
	sla::Vec2d veh_m( veh.xMatchedFrom(), veh.yMatchedFrom() );
	...
	for (set< RndfEdge* >::iterator it = edges.begin(); it != edges.end(); ++it)
	{
		...
		if ( rect_rect_X( sla::Vec2d( m.x(), m.y() ), angle(vec), edge->getWidth(), edge->getLength(),
...
//
//			// Abstand überprüfen
//			if ( squared_distance(seg, pos) > sqr( edge->getLaneWidth()/2. ) ) continue;
//
