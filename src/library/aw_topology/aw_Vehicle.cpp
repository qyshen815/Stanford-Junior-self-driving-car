...
//! maximale Suchtiefe um den Abstand eines Fahrzeugs zu einer Intersection zu ermitteln
const double MAX_INTERSECTION_FWD_SEARCH_DIST		= 50.; // [m]
const double MAX_INTERSECTION_BWD_SEARCH_DIST		= 30.; // [m]
//!  maximale Suchtiefe um den Abstand eines Fahrzeugs zu einer Stoplinie zu ermitteln
...
Vehicle::Vehicle(const PerceptionDynamicObstacle& vehicle, double offset_x, double offset_y) :
...
void Vehicle::update(const PerceptionDynamicObstacle& vehicle, double offset_x, double offset_y)
{
...
void Vehicle::match_to_graph( std::map<int, RndfEdge*>& edge_map, bool skip_graph_annotation )
{
	...
	if( edge_ ) {
		dist_to_end_ = edge_->getLength() - dist_from_start_;
...
//			}
			...
			// Multimatching
			if ( edge_->getIntersection() )
			{
				// multi matching in intersections
				edges_ = multi_match_2_graph( *this, edge_->getIntersection()->getEdges() );
...
//					TRACE(">>>>>>>>>>>>>>>>>>>>>  " << id << " to " << it->first->name());
			...
			} else
			{
				...
				for (set< RndfEdge* >::iterator it = edge_->fromVertex()->getOutEdges().begin(); it != edge_->fromVertex()->getOutEdges().end(); ++it)
					if ( edge_->isLaneChangeEdge() || (*it)->isLaneChangeEdge() ) lc_edges.insert( *it );
				for (set< RndfEdge* >::iterator it = edge_->toVertex()->getInEdges().begin(); it != edge_->toVertex()->getInEdges().end(); ++it)
					if ( edge_->isLaneChangeEdge() || (*it)->isLaneChangeEdge() ) lc_edges.insert( *it );
...
void Vehicle::rematch_in_intersection( bool skip_graph_annotation )
{
	...
	RndfIntersection* isec = edge_->getIntersection();
	map<int, RndfEdge*> edge_map;
	int i = 0;
	for (TRndfEdgeSet::const_iterator it = isec->getEdges().begin(); it != isec->getEdges().end(); ++it, ++i) {
...
//	TRACE("");
	...
	for (map<double, RndfEdge*>::iterator it = matchings.begin(); it != matchings.end(); ++it) {
		...
		double score = (it->first) * base_edge->getLength();
		cum_scores[ base_edge ] = score;
		cum_length[ base_edge ] = base_edge->getLength();
	}
	...
	for (map<double, RndfEdge*>::iterator it = matchings.begin(); it != matchings.end(); ++it) {
		...
		while( act_edge->fromVertex()->getInEdges().size() <= 1 && act_edge->isIntersectionEdge() && allowed_edges.find( act_edge ) !=  allowed_edges.end() ) {
			if (act_edge != base_edge) {
				cum_scores[ act_edge ] += score;
				cum_length[ act_edge ] += base_edge->getLength();
			}
			if ( act_edge->toVertex()->getOutEdges().size() != 1 ) break;
			RndfEdge* next_edge = *act_edge->toVertex()->getOutEdges().begin();
			act_edge = next_edge;
		}

		// rückwärts aufsummieren
		act_edge = base_edge;
		while( act_edge->toVertex()->getOutEdges().size() <= 1 && act_edge->isIntersectionEdge() && allowed_edges.find( act_edge ) !=  allowed_edges.end() ) {
			if (act_edge != base_edge) {
				cum_scores[ act_edge ] += score;
				cum_length[ act_edge ] += base_edge->getLength();
			}
			if ( act_edge->fromVertex()->getInEdges().size() != 1 ) break;
			RndfEdge* next_edge = *act_edge->fromVertex()->getInEdges().begin();
			act_edge = next_edge;
		}
	}
...
Vehicle::isOnIntersection(RndfGraph* graph) const
{
	...
	if (graph == NULL) {
		assert(vman_);
		graph = vman_->topology.complete_graph;
	}
	assert(graph);
...
Vehicle::isOnIntersection(double min_offset) const
{
	...
	assert(edge_);
	assert(vman_);
	RndfGraph* graph = vman_->topology.complete_graph;
	assert(graph);
	...
	while (sedge && sedge->isVirtualEdge()) {
		if (sedge != edge_) dist += sedge->getLength();
		sedge = graph->findIncomingEdge( sedge->fromVertex() );
	}
	RndfVertex* waypoint = sedge->toVertex();
...
//	std::cout << "  -> is on Edge ("<< edge->name() <<")"<< std::endl;
	if (edge_->getIntersection()) {
//		std::cout << "  -> is on Intersection ("<< edge->getIntersection()->getId() <<")"<< std::endl;
		if (intersection == NULL || edge_->getIntersection()->getId() == intersection->getId()) return 0.;
	}
...
//		std::cout << "    -> max search dist reached: "<< act_dist << std::endl;
	...
	if (edge.getIntersection()) {
		if (intersection == NULL || edge.getIntersection()->getId() == intersection->getId()) {
//		std::cout << "    -> Intersection ("<< edge.getIntersection()->getId() <<")"<< std::endl;
		return act_dist;
	...
	assert(waypoint);
	const RndfVertex::TEdgeSet& nextEdges = waypoint->getOutEdges();
...
//		std::cout << "    -> Intersection ("<< edge.getIntersection()->getId() <<")"<< std::endl;
	...
	assert(waypoint);
	const RndfVertex::TEdgeSet& nextEdges = waypoint->getInEdges();
	for (RndfVertex::TEdgeSet::iterator it=nextEdges.begin(); it != nextEdges.end(); ++it) {
...
double Vehicle::bwdDistToStopLine(const RndfIntersection* intersection, const RndfEdge& edge, double act_dist) const
{
	...
	if (waypoint->isStopVertex()) {
		if (waypoint->isStopVertex() && ( intersection == NULL || waypoint->hasIntersectionOutEdge(intersection)))
//		if (waypoint->isStopVertex() && (intersection == NULL || (waypoint->getIntersection() && waypoint->getIntersection()->getId() == intersection->getId())))
			return act_dist;
		else return std::numeric_limits<double>::infinity();
	}
...
{
	sla::Vec2d veh_m_1( x_matched_from_, y_matched_from_ );
	sla::Vec2d veh_m_2( veh.xMatchedFrom(), veh.yMatchedFrom() );
...
//      if(!edge) {
    pred_edge=edge();
      if(!pred_edge) {
        throw Exception("Found unmatched vehicle while determining vehicle predictions.");
      }
...
//          printf("End of graph..predicting linear.\n");
        ...
        pred_edge = bestPredictedEdge(vert->getOutEdges());
        while (predicted_dist>pred_edge->getLength()) {
          vert = pred_edge->toVertex();
          predicted_dist-=pred_edge->getLength();
          if(vert->numOutEdges()==0) {
//              printf("End of graph..predicting linear.\n");
            break;
          }
