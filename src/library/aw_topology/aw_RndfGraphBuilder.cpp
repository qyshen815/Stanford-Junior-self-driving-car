...
const double RndfGraphBuilder::max_default_edge_speed_ = dgc_mph2ms(25);  // maximum speed for edges with unspecified speed limit
...
void RndfGraphBuilder::addCheckpoints(RndfGraph& graph, const TCheckPointMap& checkpoints) {
    ...
    if(!vertex) {
      throw vlr::Exception("addCheckpoints: Could not find vertex associated with way point name " + cp->wayPoint()->name());
    }
...
RndfVertex * RndfGraphBuilder::addVertex(RndfGraph * graph, WayPoint* point) {
	...
	return graph->addVertex(point->lat(), point->lon(), point->x(), point->y(), point->name(), -1);//point->getID());
}
...
void RndfGraphBuilder::addStops(RndfGraph& graph, const TStopMap& stops) {
  for (TStopMap::const_iterator stopit = stops.begin(); stopit != stops.end(); ++stopit) {
    ...
    if(!vertex) {
      throw vlr::Exception("addStops: Could not find vertex associated with way point name.");
    }
    vertex->setStop();
  }
}
...
void RndfGraphBuilder::addTrafficLights(RndfGraph& graph, const TTrafficLightMap& tlmap) {
  for (TTrafficLightMap::const_iterator tlit =tlmap.begin(); tlit != tlmap.end(); ++tlit) {
    ...
    std::map<std::string, WayPoint*>::const_iterator wpit=tl->waypoints().begin(), wpit_end=tl->waypoints().end();
    for(; wpit!=wpit_end; wpit++) {
      RndfVertex* vertex = graph.findVertex((*wpit).second->name());
      if(!vertex) {
        throw vlr::Exception("addTrafficLights: Could not find vertex associated with way point name.");
      }
...
void RndfGraphBuilder::addCrosswalks(RndfGraph& graph, const TCrosswalkMap& cwmap) {
  for (TCrosswalkMap::const_iterator cwit=cwmap.begin(); cwit != cwmap.end(); ++cwit) {
    ...
    for(; wpit!=wpit_end; wpit++) {
      RndfVertex* vertex = graph.findVertex((*wpit).second->name());
      if(!vertex) {
        throw vlr::Exception("addCrosswalks: Could not find vertex associated with way point name.");
      }
...
void RndfGraphBuilder::addExits(RndfGraph * graph, const TExitMap* exits, double minSpeed, double maxSpeed)
{
//	printf("numExits: %i\n", exits->size());
	for (TExitMap::const_iterator exitit = exits->begin(); exitit != exits->end(); ++exitit) {
		...
		switch (myexit->exitType()) {
			case rndf::Exit::LaneToLane: {
				...
				bool isOffroad = toPointL->parentLane()->getSegment()->getOffroad();
				RndfEdge* edge = addEdge(graph, fromPointL, toPointL, "Exit_"+myexit->name(), false, minSpeed, maxSpeed, isOffroad, -1);
//				std::cout << "Exit "<< fromPointL->getID() << " "<< fromPointL->name() <<" "<< myexit->name() << std::endl;
				...
				assert(vlane);
				LaneSegment* lseg = vlane->getLaneSegment(0);
				assert(lseg);
				...
				if (lseg->intersection()) {
					IntersectionMap::iterator it_intersection = graph->intersectionMap.find( lseg->intersection()->getID() );
			...
			case rndf::Exit::LaneToPerimeter: {
				...
				bool isOffroad = toPointP->getPerimeter()->zone()->getOffroad();
				RndfEdge * edge = addEdge(graph, fromPointL, toPointP, "Exit_"+myexit->name(), false, minSpeed, maxSpeed, isOffroad, -1);
//				std::cout << "Exit "<< fromPointL->getID() << " "<< toPointP->name() <<" "<< myexit->name() << std::endl;
				assert(graph->findEdge("Exit_"+myexit->name()) != NULL);
				graph->findVertex(fromPointL->name())->setExit();
				edge->setZone();
				break;
			...
			case rndf::Exit::PerimeterToLane: {
				...
				WayPoint * toPointL = myexit->getExitToLane();
				bool isOffroad = toPointL->parentLane()->getSegment()->getOffroad();
				RndfEdge * edge = addEdge(graph, fromPointP, toPointL, "Exit_"+myexit->name(), false, minSpeed, maxSpeed, isOffroad, -1);//myexit->getID());
//				std::cout << "Exit "<< fromPointP->getID() << " "<< fromPointP->name() << std::endl;
...
RndfEdge * RndfGraphBuilder::addEdge(RndfGraph * graph, const WayPoint * p1, const WayPoint * p2, string name,
		bool isLane, double minSpeed, double maxSpeed, bool isOffroad, int id)
{
	...
	return graph->addEdge(fromVertex, toVertex, name, isLane, false, minSpeed, maxSpeed, isOffroad, computeWeight(fromVertex, toVertex,
			maxSpeed), id);
}
...
RndfEdge * RndfGraphBuilder::addEdge(RndfGraph * graph, const PerimeterPoint * p1, const PerimeterPoint * p2,
		string name, bool isLane, double minSpeed, double maxSpeed, bool isOffroad, int id) {
	RndfVertex * fromVertex = graph->findVertex(p1->name());
	assert(fromVertex != NULL);
	RndfVertex * toVertex = graph->findVertex(p2->name());
	assert(toVertex != NULL);
...
void RndfGraphBuilder::connectAll(RndfGraph * graph, const TWayPointMap* waypoints, string prefix,
		double minSpeed, double maxSpeed, bool isOffroad, bool zone_, double width) {
	for (TWayPointMap::const_iterator it1 = waypoints->begin(); it1 != waypoints->end(); ++it1) {
		for (TWayPointMap::const_iterator it2 = waypoints->begin(); it2 != waypoints->end(); ++it2) {
			...
			if (p1 == p2)
				continue;
			RndfEdge * edge = addEdge(graph, p1, p2, prefix+"_"+p1->name()+"_"+p2->name(), false, minSpeed, maxSpeed, isOffroad, -1);
			if (zone_) {
				edge->setZone();
			}
			if (width > 0) {
...
void RndfGraphBuilder::connectAll(RndfGraph * graph, const TWayPointMap* waypoints, const WayPoint * p2,
		string prefix, double minSpeed, double maxSpeed, bool isOffroad, bool zone_) {
	for (TWayPointMap::const_iterator it1 = waypoints->begin(); it1 != waypoints->end(); ++it1) {
		WayPoint * p1 = (*it1).second;
		if (p1 == p2)
			continue;
		RndfEdge * edge12 = addEdge(graph, p1, p2, prefix+"_"+p1->name()+"_"+p2->name(), false, minSpeed, maxSpeed,
		    isOffroad, -1);
		RndfEdge * edge21 = addEdge(graph, p2, p1, prefix+"_"+p2->name()+"_"+p1->name(), false, minSpeed, maxSpeed,
		    isOffroad, -1);
		if (zone_) {
			edge12->setZone();
			edge21->setZone();
...
void RndfGraphBuilder::connectAll(RndfGraph * graph, const TWayPointVec* waypoints, string prefix,
  ...
  for (TWayPointVec::const_iterator it1 = waypoints->begin(); it1 != waypoints->end(); ++it1) {
    for (TWayPointVec::const_iterator it2 = waypoints->begin(); it2 != waypoints->end(); ++it2) {
      ...
      if (p1 == p2)
        continue;
      RndfEdge * edge = addEdge(graph, p1, p2, prefix+"_"+p1->name()+"_"+p2->name(), false, minSpeed, maxSpeed,
          isOffroad, -1);
      if (zone_) {
        edge->setZone();
      }
...
void RndfGraphBuilder::connectAll(RndfGraph * graph, const TWayPointVec* waypoints, const WayPoint * p2,
  ...
  for (TWayPointVec::const_iterator it1 = waypoints->begin(); it1 != waypoints->end(); ++it1) {
    ...
    if (p1 == p2)
      continue;
    RndfEdge * edge12 = addEdge(graph, p1, p2, prefix+"_"+p1->name()+"_"+p2->name(), false, minSpeed, maxSpeed, isOffroad, -1);
    RndfEdge * edge21 = addEdge(graph, p2, p1, prefix+"_"+p2->name()+"_"+p1->name(), false, minSpeed, maxSpeed, isOffroad, -1);
    if (zone_) {
      edge12->setZone();
      edge21->setZone();
    }
...
void RndfGraphBuilder::connectAll(RndfGraph * graph, const TPerimeterPointMap* perimeterpoints, string prefix,
		double minSpeed, double maxSpeed, bool isOffroad) {
...
void RndfGraphBuilder::connectAll(RndfGraph * graph, const TPerimeterPointMap* perimeterpoints,
		const WayPoint * p2, string prefix, double minSpeed, double maxSpeed, bool isOffroad) {
	for (TPerimeterPointMap::const_iterator it1 = perimeterpoints->begin(); it1 != perimeterpoints->end(); ++it1) {
		PerimeterPoint * p1 = (*it1).second;
		if ((WayPoint*)p1 == p2)
			continue;
...
RndfGraph* RndfGraphBuilder::buildGraph()
{
	...
	for (intit = intersections.begin(),intit_end = intersections.end(); intit != intit_end; ++intit) {
		...
		graph->addIntersection(intersection->getID());
	}
	...
	for (TLaneMap::const_iterator laneit = network->lanes().begin(); laneit != network->lanes().end(); ++laneit) {
		...
		double maxSpeed = getMaxSpeed(lane);
		double minSpeed = getMinSpeed(lane);
		bool isOffroad = lane->getSegment()->getOffroad();
		double width = lane->getLaneWidth(); // * FT2M_FACTOR;
		const rndf::TLaneSegmentVec& lane_segments = lane->getLaneSegments();
...
//			cout << "  Com Weight: "<< "Lane_"+lane->name()+"_"+current_waypoint->name()+"_"+next_waypoint->name() << endl;
			edge = graph->addEdge(current_vertex, next_vertex, "Lane_"+lane->name()+"_"+current_waypoint->name()+"_"+next_waypoint->name(), true, false, minSpeed, maxSpeed, isOffroad, computeWeight(
					current_vertex, next_vertex, maxSpeed), -1);
			edge->setWidth(width);
			edge->setLeftBoundary(lane->getLeftBoundaryType());
			edge->setRightBoundary(lane->getRightBoundaryType());
			edge->setStopLane( (*its)->isStopLane() );

			// check if Lane segment belongs to an intersection
			if ((*its)->intersection()) {
				// add edge to intersection
				IntersectionMap::iterator it_intersection = graph->intersectionMap.find((*its)->intersection()->getID());
				RndfIntersection* intersection = it_intersection->second;
				intersection->addEdge(edge);
  ...
  addAdjacentLanes(*graph, network->lanes());
    ...
    printf("Adding zone %s to graph\n", zone->name().c_str());
		double minSpeed = getMinSpeed(zone);
		double maxSpeed = getMaxSpeed(zone);
		bool isOffroad = zone->getOffroad();
...
//      addCheckpoints(*graph, myspot->checkPoints());
      connectAll(graph, &spot->wayPoints(), "Spot_"+spot->name(), minSpeed, maxSpeed, isOffroad, true, spot->getSpotWidth());
    }
			...
			for (TPerimeterMap::const_iterator perimeterit = zone->perimeters().begin(); perimeterit != zone->perimeters().end(); ++perimeterit) {
				...
				for (TPerimeterPointVec::const_iterator ppointit = myperimeter->perimeterPoints().begin(); ppointit != myperimeter->perimeterPoints().end(); ++ppointit) {
					...
					if (mypoint->exits().size()) {
						...
						RndfEdge * edge = addEdge(graph, point1, point2, "Zone_p2ex_"+point2->name()+"_"+point1->name(), false, minSpeed, maxSpeed, isOffroad, -1);
						edge->setZone();
					}
					if (mypoint->entries().size()) {
						PerimeterPoint * point2 = mypoint;
						assert(point2); // TODO: connections to other zones?
						RndfEdge * edge = addEdge(graph, point2, point1, "Zone_p2en_"+point1->name()+"_"+point2->name(), false, minSpeed, maxSpeed, isOffroad, -1);
						edge->setZone();
					}
				}
			}
				...
				if ( mypoint->entries().size()) {
					for (TPerimeterPointVec::const_iterator ppointit2 = myperimeter->perimeterPoints().begin(); ppointit2 != myperimeter->perimeterPoints().end(); ++ppointit2) {
						...
						if (mypoint2->exits().size()) {
							RndfEdge * edge = addEdge(graph, mypoint, mypoint2, "Zone_p2p_en_"+mypoint->name()+"_"+mypoint2->name(), false, minSpeed, maxSpeed, isOffroad, -1);
							edge->setZone();
						}
					}
				}
  ...
  for (TLaneMap::const_iterator laneit = network->lanes().begin(); laneit != network->lanes().end(); ++laneit) {
    ...
    addExits(graph, &mylane->exits(), getMinSpeed(mylane), getMaxSpeed(mylane));
  }

  // add Zone exits
  for (TPerimeterMap::const_iterator perimeterit = network->perimeters().begin(); perimeterit != network->perimeters().end(); ++perimeterit) {
    Perimeter* myperimeter = perimeterit->second;
    addExits(graph, &myperimeter->exits(), getMinSpeed(myperimeter->zone()), getMaxSpeed(myperimeter->zone()));
  }
...
// TODO: find were they are used...
  ...
  for (TLaneMap::const_iterator laneit = network->lanes().begin(); laneit != network->lanes().end(); ++laneit) {
    ...
    for (rndf::TLaneSegmentVec::const_iterator it=mylane->getLaneSegments().begin(); it != mylane->getLaneSegments().end(); ++it) {
...
//        cout << "  "<< edge_name << "  <->  "<< c_edge_name << endl;
        ...
        edge->crossingEdges.insert(c_edge);
        c_edge->crossingEdges.insert(edge);
      }
    }
  }
  cout << "Ok" << endl;;
...
void RndfGraphBuilder::addAdjacentLanes(RndfGraph& graph, const TLaneMap& lanes) {
  ...
  for (TLaneMap::const_iterator laneit = lanes.begin(); laneit != lanes.end(); ++laneit) {
      ...
      for (rndf::TLaneSegmentVec::const_iterator it=lane->getLaneSegments().begin(); it != lane->getLaneSegments().end(); ++it) {
          rndf::LaneSegment* lseg = *it;
          ...
          for (rndf::TLaneSegmentSet::iterator l_it=lseg->oncomingLaneSegments().begin(); l_it != lseg->oncomingLaneSegments().end(); ++l_it) {
              ...
              edge->leftOppositeEdges.insert(n_edge);
          }
      }
  }
}

double RndfGraphBuilder::getMaxSpeed(Lane* lane) {
  const SpeedLimit* sl = lane->getSpeedLimit();
  ...
  Segment* segment = lane->getSegment();
	...
	sl = segment->getSpeedLimit();
  if(sl) {return sl->maxSpeed();}

  return max_default_edge_speed_;
}

double RndfGraphBuilder::getMinSpeed(Lane* lane) {
  const SpeedLimit* sl = lane->getSpeedLimit();
...
double RndfGraphBuilder::getMaxSpeed(Zone* zone) {
	const SpeedLimit* sl = zone->getSpeedLimit();
  if(sl) {return sl->maxSpeed();}
  return max_default_edge_speed_;
}

double RndfGraphBuilder::getMinSpeed(Zone* zone) {
  const SpeedLimit* sl = zone->getSpeedLimit();
  if(sl) {return sl->minSpeed();}
  return 0.0;
}
...
RndfEdge* RndfGraphBuilder::addLaneChangeEdge(RndfGraph * graph, rndf::WayPoint const * p1, rndf::WayPoint const * p2, double minSpeed, double maxSpeed, bool isOffroad)
{
	if (p1 == NULL || p2 == NULL) {
...
//		cout << "LaneChange Lane to long: "<< length <<"  (skipped)" << endl;
	...
	double weight = computeWeight(fromVertex, toVertex, maxSpeed) * LANECHANGE_EDGE_WEIGHT_FACTOR;  // Weight increased to avoid unnecessary Lane changes
	return graph->addEdge(fromVertex, toVertex, "LaneChange_"+p1->name()+"_"+p2->name(), false, true, minSpeed, maxSpeed, isOffroad, weight, -1);
}
...
void RndfGraphBuilder::addLaneChangeEdges(RndfGraph * graph) {
  for (rndf::TLaneMap::const_iterator laneit = network->lanes().begin(); laneit != network->lanes().end(); ++laneit) {
    ...
    double maxSpeed = getMaxSpeed(lane);
    bool isOffroad = lane->getSegment()->getOffroad();
    for (rndf::TLaneSegmentVec::const_iterator segit = lane->getLaneSegments().begin(); segit != lane->getLaneSegments().end(); ++segit) {
      LaneSegment* seg = *segit;
      RndfEdge* edge_1 = graph->findEdge("Lane_" + lane->name() + "_" + seg->fromWayPoint()->name() + "_" + seg->toWayPoint()->name());
      if (!edge_1) {
        throw Exception("addLaneChangeEdges: Graph is inconsistent - cannot find edge Lane_" + lane->name() + "_" + seg->fromWayPoint()->name() + "_" + seg->toWayPoint()->name());
      }
      ...
      if (!seg->leftLaneSegments().empty()) {
        for (TLaneSegmentSet::iterator sit = seg->leftLaneSegments().begin(); sit != seg->leftLaneSegments().end(); ++sit) {
          ...
          if (lane->getLeftBoundaryType() == Lane::DoubleYellow) continue;
          if (lane->getLeftBoundaryType() == Lane::SolidWhite) continue;
          //					std::cout << seg->name() << " has left Lane " << seg2->name() << std::endl;
          RndfEdge* edge = addLaneChangeEdge(graph, seg->fromWayPoint(), seg2->toWayPoint(), minSpeed, maxSpeed,
              isOffroad);
          if (edge) {
            ...
            if (!edge_2) {
              throw Exception("addLaneChangeEdges: Graph is inconsistent - cannot find edge Lane_"
                  + seg2->lane()->name() + "_" + seg2->fromWayPoint()->name() + "_" + seg2->toWayPoint()->name());
            }
            edge->leftEdges.insert(edge_2);
            //edge_2->rightEdges.insert( edge );
            edge->rightEdges.insert(edge_1);
      ...
      if (!seg->rightLaneSegments().empty()) {
        for (TLaneSegmentSet::iterator sit = seg->rightLaneSegments().begin(); sit != seg->rightLaneSegments().end(); ++sit) {
          LaneSegment * seg2 = *sit;
          if (seg2->lane()->isVirtual()) continue;
          if (lane->getRightBoundaryType() == Lane::DoubleYellow) continue;
          if (lane->getRightBoundaryType() == Lane::SolidWhite) continue;
          ...
          if (edge) {
            ...
            RndfEdge* edge_2 = graph->findEdge("Lane_" + seg2->lane()->name() + "_" + seg2->fromWayPoint()->name()
                + "_" + seg2->toWayPoint()->name());
            assert(edge_2);
            edge->rightEdges.insert(edge_2);
            //edge_2->leftEdges.insert( edge );
            edge->leftEdges.insert(edge_1);
      ...
      if (!seg->oncomingLaneSegments().empty()) {
        ...
        //					laneSegment * seg2 = *sit;
        //					if (seg2->lane()->isVirtual()) continue;
        //					if (seg2->lane()->getLeftBoundaryType() == Lane::DoubleYellow) continue;
        //					std::cout << seg->name() << " has opposite Lane " << (*(seg->oncomingLaneSegments().begin()))->name() << std::endl;
        //					RndfEdge* edge = addLaneChangeEdge(graph, seg->fromWayPoint(), seg2->fromWayPoint(), minSpeed, maxSpeed);
...
void RndfGraphBuilder::addVirtualLanes(RndfGraph * graph)
{
  ...
  for (TLaneMap::const_iterator laneit = network->lanes().begin(); laneit != network->lanes().end(); ++laneit) {
		Lane * mylane = laneit->second;
		addVirtualLanes(graph, mylane, getMinSpeed(mylane), getMaxSpeed(mylane));
	}
  // for all perimeters
  for (TPerimeterMap::const_iterator perimeterit = network->perimeters().begin(); perimeterit != network->perimeters().end(); ++perimeterit) {
    Perimeter * myperimeter = perimeterit->second;
    addVirtualLanes(graph, myperimeter, getMinSpeed(myperimeter->zone()), getMaxSpeed(myperimeter->zone()));
  }

}

void RndfGraphBuilder::addVirtualLanes(RndfGraph* graph, Lane* mylane, double minSpeed, double maxSpeed) {
...
void RndfGraphBuilder::addVirtualLanes(RndfGraph * graph, Perimeter* myperimeter, double minSpeed, double maxSpeed)
{
  for (TExitMap::const_iterator exitit = myperimeter->exits().begin(); exitit != myperimeter->exits().end(); ++exitit)
  {
    ...
    if (oldEdge->isZoneEdge()) {
      ...
      for (set< RndfEdge* >::iterator it = fromVertex->getInEdges().begin(); it != fromVertex->getInEdges().end(); ++it) {
		if ( ! (*it)->isVirtualEdge() ) {
			prevEdge = *it;
			break;
		}
...
//      graph->findIncomingEdge(fromVertex);
      ...
      for (set< RndfEdge* >::iterator it = toVertex->getOutEdges().begin(); it != toVertex->getOutEdges().end(); ++it) {
		if ( ! (*it)->isVirtualEdge() ) {
			followingEdge = *it;
			break;
		}
...
//     }
      ...
      // Curve smoothen
      graph->addVirtualLane(oldEdge, firstVertex, fromVertex, toVertex, lastVertex, myexit->name(), minSpeed, maxSpeed, this);
    }
  }
}
