...
PolyTraj2D::PolyTraj2D( const std::map<double, CurvePoint>& /*center_line*/, const PolyTraj& traj_lat,
...
TrajectoryPoint2D PolyTraj2D::latLongTrajectories2GlobalCordinates(const trajectory_point_1D& lat_traj_point, 
    ...
    if (  ( - DBL_EPSILON < one_minus_kappa_d ) && (  one_minus_kappa_d < DBL_EPSILON ) ) {
    	vlr::Exception except_( "The calculated trajectory is too far off from center line for its curvature!");
    	throw except_;
    }
   ...
   const double s_dot_square = s_dot * s_dot;
	...
	if ( ( delta_theta <= - M_PI_2 ) || (  M_PI_2 <= delta_theta ) ) {
		vlr::Exception except_( "The calculated heading is too far off from the center line!");
	    throw except_;
	}
...
/*static*/ void PolyTraj2D::globalCordinates2LatLong(const TrajectoryPoint2D& tp_2d, 
        const std::map<double, CurvePoint>& center_line, const GenerationMode& generation_mode,
    ...
    if ( center_line.size() <= 1 ) {
		vlr::Exception except_( "No entries in center line!");
	    throw except_;
    }
    std::map<double, CurvePoint>::const_iterator it_closest = center_line.begin();
    double min_dist_sq = DBL_MAX;
    double dist_sq;
    
    // Find closest point as start solution
    std::map<double, CurvePoint>::const_iterator it_cl_point;
    for ( it_cl_point = center_line.begin();
            it_cl_point != center_line.end(); 
            it_cl_point++ ) {

        const double& r1 = it_cl_point->second.x;
        const double& r2 = it_cl_point->second.y;
    ...
    double s0 = it_closest->second.s;
    double s1;
    
    if ( it_closest != center_line.begin() ) {
        it_closest--;
        s1 = it_closest->second.s;
    }
    else {
        it_closest++;
        s1 = it_closest->second.s;
    ...
    try {
 	...
 	vlr::Exception except_( "Secant method -> " + except.getErrorMessage());
 	throw except_;
    }
    ...
    do {
        ...
        if ( ( - DBL_EPSILON < delta_f ) && ( delta_f < DBL_EPSILON ) ) {
         	vlr::Exception except_( "Secant method: too many iterations?" );
         	throw except_;
        }
        ...
        try {
      	...
      	vlr::Exception except_( "Secant method: f2 = f(s, x, y) -> " + except.getErrorMessage());
      	throw except_;
        }
        ...
        count--;
        if (count == 0) {
          	vlr::Exception except_( "Secant method has not converged fast enough!");
          	throw except_;
        }
    ...
    if (  (delta_theta >= M_PI_2) || ( delta_theta  <= -M_PI_2 )  ) {
      ...
      throw vlr::Exception( "Projection failed! Reference heading is too far off!");
    }
...
/*static*/ void PolyTraj2D::projectionCondition(const std::map<double, CurvePoint>& center_line, const double& s_foot,
	...
	vlr::Exception except_( "Polynomial trajectory -> " + except.getErrorMessage());
	throw except_;
   }
...
void PolyTraj2D::calculateNextInitialStates(const double& t_next_deadline,
    ...
    if ( t_last_deadline > t_next_deadline ) {
      	vlr::Exception except_( "t_last_deadline > t_next_deadline!" );
      	throw except_;
    }
...
TrajectoryPoint2D PolyTraj2D::calculateNextStartTrajectoryPoint(const double& t_next) const {
...
void PolyTraj2D::generateNewControlTrajectory(const double& t_current, const double& t_control_horizon,
                                              const double& control_t_res, std::vector<TrajectoryPoint2D>& trajectory) const {
