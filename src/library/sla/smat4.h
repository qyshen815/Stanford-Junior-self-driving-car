...
/** 
 * @file mat.h
 * @brief Basic static square 4x4 matric type, templated in type and size
...
namespace sla {
  ...
  public:
    ...
    // Named Constructors
    static SMat4<T> fromRows(const Vec4<T>& q0, const Vec4<T>& q1, 
			     const Vec4<T>& q2, const Vec4<T>& q3);
    static SMat4<T> fromCols(const Vec4<T>& q0, const Vec4<T>& q1, 
			     const Vec4<T>& q2, const Vec4<T>& q3);
    ...
    void set(const T x11, const T x12, const T x13, const T x14,
	     ...
	     const T x31, const T x32, const T x33, const T x34,
	     const T x41, const T x42, const T x43, const T x44);
    void setRows(const Vec4<T>& q0, const Vec4<T>& q1, 
		 const Vec4<T>& q2, const Vec4<T>& q3);
    void setCols(const Vec4<T>& q0, const Vec4<T>& q1, 
		 const Vec4<T>& q2, const Vec4<T>& q3);
  }; // end class SMat4<T>
  ...
  inline void SMat4<T>::set(const T x11,const T x12,const T x13,const T x14,
			    ...
			    const T x31,const T x32,const T x33,const T x34,
			    const T x41,const T x42,const T x43,const T x44){
    x[0] = x11; x[4] = x12; x[8] = x13;  x[12] = x14;
    x[1] = x21; x[5] = x22; x[9] = x23;  x[13] = x24;
    x[2] = x31; x[6] = x32; x[10] = x33; x[14] = x34;
    x[3] = x41; x[7] = x42; x[11] = x43; x[15] = x44;
  }
