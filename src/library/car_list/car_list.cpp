  ...
  for(i = 0; i < num_cars(); i++) {
    ...
    for(j = 0; j < car[i].num_poses(); j++)
      fprintf(fp, "1 %f %f %f %d %d %lf ", car[i].pose[j].x, car[i].pose[j].y,
          car[i].pose[j].theta, (int)car[i].pose[j].signal, car[i].pose[j].t, car[i].pose[j].timestamp);
    fprintf(fp, "\n");
  }
  fclose(fp);
}
  ...
  for(i = 0; i < num_poses(); i++) {
    if(t == pose[i].timestamp) {
      ...
      *y = pose[i].y + u * (pose[i + 1].y - pose[i].y);
      dtheta = dgc_normalize_theta(pose[i + 1].theta - pose[i].theta);
...
bool car_t::estimate_signal(double t, TurnSignalState* signal)
{
  int i;
...
bool car_t::estimate_signal(int t, TurnSignalState* signal)
{
  int i;
...
bool car_t::car_selected(double scene_x, double scene_y, int t)
{
  ...
  x2  = x - (-w / 2.0) * stheta;
  y2  = y + (-w / 2.0) * ctheta;
  dgc_point_to_line_distance(scene_x, scene_y, x1, y1, x2, y2, &par_dist,
