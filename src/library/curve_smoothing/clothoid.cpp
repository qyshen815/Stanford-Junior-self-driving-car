...
void Clothoid::SpiroCPsToBezier(spiro_cp* spiros, int n, int isclosed, BezierContext* bc) {
  spiro_seg *s;
...
void Clothoid::TaggedSpiroCPsToBezier(spiro_cp* spiros, BezierContext* bc) {
  spiro_seg* s;
  int n;
...
void Clothoid::integrate_spiro(const double ks[4], double xy[2]) {
  double th1 = ks[0];
  double th2 = .5 * ks[1];
  double th3 = (1. / 6) * ks[2];
...
double Clothoid::compute_ends(const double ks[4], double ends[2][4], double seg_ch) {
  double xy[2];
  double ch, th;
  double l, l2, l3;
...
void Clothoid::compute_pderivs(const spiro_seg *s, double ends[2][4], double derivs[4][2][4], int jinc) {
  double recip_d = 2e6;
...
double Clothoid::mod_2pi(double th) {
  double u = th / (2 * M_PI);
  return 2 * M_PI * (u - floor(u + 0.5));
}

Clothoid::spiro_seg* Clothoid::setup_path(const Clothoid::spiro_cp* src, int n) {
...
void Clothoid::bandec11(bandmat *m, int *perm, int n) {
  int i, j, k;
  int l;
...
void Clothoid::banbks11(const bandmat *m, const int *perm, double *v, int n) {
  int i, k, l;
...
int Clothoid::compute_jinc(char ty0, char ty1) {
...
int Clothoid::count_vec(const spiro_seg *s, int nseg) {
  int i;
  int n = 0;
...
void Clothoid::add_mat_line(bandmat *m, double *v, double derivs[4], double x, double y, int j, int jj, int jinc, int nmat) {
  int k;
...
double Clothoid::spiro_iter(spiro_seg *s, bandmat *m, int *perm, double *v, int n) {
...
int Clothoid::solve_spiro(spiro_seg *s, int nseg) {
  bandmat *m;
  double *v;
  int *perm;
...
void Clothoid::spiro_seg_to_bpath(const double ks[4], double x0, double y0, double x1, double y1, BezierContext *bc, int depth) {
...
Clothoid::spiro_seg* Clothoid::run_spiro(const Clothoid::spiro_cp *src, int n) {
  int nseg = src[0].ty == '{' ? n - 1 : n;
  Clothoid::spiro_seg *s = setup_path(src, n);
  if (nseg > 1) solve_spiro(s, nseg);
  return s;
}

void Clothoid::free_spiro(spiro_seg *s) {
  free(s);
}

void Clothoid::spiro_to_bpath(const spiro_seg* s, int n, BezierContext* bc) {
  int i;
  ...
  for (i = 0; i < nsegs; i++) {
    ...
    if (i == 0) bc->moveto(x0, y0, s[0].ty == '{');
    spiro_seg_to_bpath(s[i].ks, x0, y0, x1, y1, bc, 0);
  }
}

double Clothoid::get_knot_th(const spiro_seg *s, int i) {
  double ends[2][4];
