...
void draw_limit(double x, double y, double theta, double v)
{
  ...
  glTranslatef(x, y, 0);
  sprintf(str, "%.1f", v);
  angle = dgc_normalize_theta(theta);
...
{
  path->num_points = num_points;
  path->points = (dgc_point2D_p)calloc(path->num_points, 
              ...
              double smooth_x, double smooth_y, double smooth_z)
{
  ...
  glPushMatrix();
  if(gls->coordinates == GLS_GLOBAL_COORDINATES) {
    glTranslatef(gls->origin_x - origin_x,
		 gls->origin_y - origin_y, 
		 gls->origin_z - origin_z);
  }
  else if(gls->coordinates == GLS_SMOOTH_COORDINATES) {
...
void draw_coordinate_frame(double scale) {
  ...
  const int a_modifyMaterialState = TRUE;
