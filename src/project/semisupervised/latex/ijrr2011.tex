...
In this paper, we consider a semi-supervised approach to the problem of track classification in dense 3D range data.  This problem involves the classification of objects that have been segmented and tracked without the use of a class-specific tracker.  This paper is an extended version of \citep{Teichman2011b}.

We propose a method based on the EM algorithm: iteratively 1) train a classifier, and 2) extract \textit{useful} training examples from unlabeled data by exploiting tracking information.  We evaluate our method on a large multiclass problem in dense LIDAR data collected from natural street scenes.
...
Currently, object recognition capabilities are a fundamental limitation of many practical robotic systems.  Besides the standard difficulty of creating \textit{reliable} object recognition systems, most systems must train on large quantities of hand-labeled data to be effective.  This need for labeled data presents a significant barrier to the use of robots in everyday settings; it is probably unreasonable to expect average users to spend large amounts of time preparing hand-labeled datasets of new objects.  We aim at making progress towards eliminating this barrier.
...
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{11-08-28_ijrr_static_imagery/ssl_intuition/ssl_intuition-small.png}
...
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{11-08-28_ijrr_static_imagery/data/data-small.jpg}
...
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{img/synthetic_example_output/track_structure.pdf}
...
\begin{figure*}
  \centering
  \includegraphics[width=0.45\linewidth]{img/synthetic_example_output/self-learning_classification_epoch_000.pdf}
  \includegraphics[width=0.45\linewidth]{img/synthetic_example_output/tbssl_classification_epoch_000.pdf} \\
  \includegraphics[width=0.45\linewidth]{img/synthetic_example_output/self-learning_classification_epoch_002.pdf}
  \includegraphics[width=0.45\linewidth]{img/synthetic_example_output/tbssl_classification_epoch_002.pdf} \\
  \includegraphics[width=0.45\linewidth]{img/synthetic_example_output/self-learning_classification_epoch_015.pdf}
  \includegraphics[width=0.45\linewidth]{img/synthetic_example_output/tbssl_classification_epoch_006.pdf}
...
\begin{figure*}
  \centering
  \includegraphics[width=\linewidth]{11-08-28_ijrr_static_imagery/pipeline.pdf}
...
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{11-08-28_ijrr_static_imagery/vocii.pdf}
...
\begin{figure*}
  \centering
  \includegraphics[width=\linewidth]{11-08-28_ijrr_static_imagery/tracking_error/tracking_error-small.png}
...
\begin{figure}
  \centering
    \includegraphics[width=0.75\linewidth]{11-08-28_ijrr_static_imagery/segmentation_error-blurred.png}
%\includegraphics[width=0.75\linewidth]{11-05-30_rss_static_img/segmentation_error_2.png}
...
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{11-08-28_ijrr_static_imagery/example_objects/example_objects-small.png}
...
Negative examples are shown as x's; large symbols indicate seed labels, and small symbols indicate negative examples provided without labels to the semi-supervised learning algorithm.  Positive examples are shown as o's; large green symbols indicate seed labels, and small white symbols represent positive examples provided without labels to the semi-supervised learning algorithm.


   First, all training examples are sorted accorded to their distance from $x_K$. Let $\Theta_K = \{ \frac{(x_K - x_m) + (x_K - x_{m-1})}{2} : m = 1 \dots M-1\}$ be the set of all radii to be considered

To ensure that the baseline classifier was unfairly represented because of overfitting (\ie, it might have done better with fewer weak classifiers) we plotted its test set performance as the number of weak classifiers was varied.  No overfitting was found; these results are shown in Figure~\ref{fig:overfitting}.

\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{img/overfitting_test.pdf}
  \caption{Even with very large numbers of weak classifiers, the boosting classifier does not overfit.  This eliminates one possible source of error in the comparisons.}
...
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{img/synthetic_example_output/tbssl_classification_epoch_006.pdf}
  \caption{The final state of tracking-based semi-supervised learning when applied to the data in Figure~\ref{fig:synth_track_structure}.  Black indicates a true negative and green a true positive.  The algorithm is able to ``follow'' the tracking structure of the data and propagate positive labels to all clusters of positive, unlabeled examples.}
  \label{fig:synth_tbssl}
\end{figure}
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{img/synthetic_example_output/self-learning_classification_epoch_015.pdf}
  \caption{The final state of semi-supervised learning when tracking information is ignored.  Red indicates a false positive; white circles are positive examples to which the algorithm was unable to assign a label.  Positive labels are unable to be spread beyond the cluster in which the seed examples were initially given.}
  \label{fig:synth_self_learning}
\end{figure}
\end{comment}


  In the following experiments, the usage of $\T_0$ varies - sometimes it is used as labeled data, sometimes as unlabeled data, and sometimes as seed data - whereas the usage of the $\B_0$ and the test set is always the same.

While boosting is known for being resistant to overfitting, this is potentially a somewhat extreme case.  However, overfitting was tested for and none was found.


Fortunately, there is an obvious adaptation of Algorithm~\ref{alg:ssl} to the online setting.
