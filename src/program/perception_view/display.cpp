...
display( void )
{
  ...
  // do not receive pose messages while drawing
  ...
  if (rndf_generate_display_lists) {
  ...
  car_pose = *applanix_current_pose();
  ...
  if (show_imagery) {
    ...
    glPushMatrix();
    {
      ...
      dgc_imagery_draw_3D(imagery_root, gui3D.camera_pose.distance,
          ...
          gui3D.camera_pose.y_offset, global_x, global_y,
          utmzone, show_flat_imagery, 1.0, 1);
    }
    glPopMatrix();
  }
  ...
  if(rndf_valid && show_rndf) {
    glPushMatrix();
    {
      if (!plain_mode) {glTranslatef(0, 0, 0.05-DGC_PASSAT_HEIGHT);}
      //          draw_rndf(*rn, 0, 1, 1, 1, 0, 1, global_x, global_y, 0.5 );
	...
	double origin_x, double origin_y);
      ...
      draw_rndf_display_list(rndf_dl, 0, 1, 1, 1, 0, 0, global_x, global_y);

      pthread_mutex_lock( &stop_zone_mutex );
      draw_stop_zones(*rn, global_x, global_y);
      pthread_mutex_unlock( &stop_zone_mutex );

      draw_trafficlights(*rn, global_x, global_y);
    }
    glPopMatrix();
  }
  ...
  if (show_obstacles && obstacles.num_points>0) {
    ...
    glPushMatrix();
    {
      ...
      for(i = 0; i < obstacles.num_points; i++) {
        if (show_dynamic && obstacles.point[i].type!=DGC_STATIC_OBSTACLE) {
          if (show_inverse) {
            rgb2.r = 0.0;
            rgb2.g = 0.0;
            rgb2.b = 1.0;
  ...
  if(show_track_classifications && obstacles.num_dynamic_obstacles>0) {
	...
	default:
    float conf = max(0.0, min(1.0, *(unsigned char*)&obstacles.dynamic_obstacle[i].confidence / 256.0));
  ...
  if(show_frame_classifications && obstacles.num_dynamic_obstacles>0) {
    glPushMatrix();
    {
      pthread_mutex_lock( &obstacles_mutex );       
          ...
          for(i = 0; i < radar_lrr3[r].num_targets; i++) {
            ...
            if (show_radar_cal) {
              ...
              if (show_all_radars ||
                ...
                rgb2.r = radar_lrr3[r].target[i].prob_obstacle;
                rgb2.g = 1 - radar_lrr3[r].target[i].prob_obstacle;
               
                draw_radar_spot( px, py, 0.0,
                    RADAR_MARKER_SIZE,
                    0.2, rgb1, rgb2, radar_lrr3[r].target[i].prob_exist );
              }
            }
          }
          pthread_mutex_unlock( &radar_mutex );
  ...
  if (!show_no_car) {
    if (!show_simulator) {
      if( last_can_ts != 0 ) {
        ...
        wheel_angle = dgc_normalize_theta( wheel_angle + dangle);
      }
  ...
  if (show_estop) {
    if (show_fancy_estop) {
      if (car_pose.timestamp - estop.timestamp < 3.0) {
        if(estop.estop_code == DGC_ESTOP_PAUSE) {
          ...
          glColor3f(1, 1, 1);
          glBegin(GL_QUADS);
          glVertex2f(2,   gui3D.window_height-2);
          glVertex2f(153, gui3D.window_height-2);
          glVertex2f(153, gui3D.window_height-33);
          glVertex2f(2,   gui3D.window_height-33);
          glEnd();
          run_color.v += run_color_move;
          if (run_color.v<0.45 || run_color.v>1.0)
            run_color_move *= -1;
          rgb1 = dgc_hsv_to_rgb(run_color);
        ...
        glVertex2f(150, gui3D.window_height-30);
        glVertex2f(5,   gui3D.window_height-30);
        glEnd();
        if (estop.estop_code == DGC_ESTOP_RUN) {
          glColor3f(1, 1, 1);
        } else {
          glColor3f(0, 0, 0);
        }
        renderBitmapString(10,gui3D.window_height-25,
      ...
      if (car_pose.timestamp - estop.timestamp < 3.0) {
        if(estop.estop_code == DGC_ESTOP_PAUSE) {
          ...
          glBegin(GL_QUADS);
          glVertex2f(5,   gui3D.window_height-5);
          glVertex2f(105, gui3D.window_height-5);
          glVertex2f(105, gui3D.window_height-25);
          glVertex2f(5,   gui3D.window_height-25);
          glEnd();
          glColor3f(1,0,0);
        } else {
          sprintf(str, "        NONE");
        ...
        renderBitmapString(5,gui3D.window_height-20,
  ...
  if (show_clock) {
    ...
    glBegin(GL_QUADS);
    {
      glVertex2f(gui3D.window_width-85,  2);
      glVertex2f(gui3D.window_width-2,   2);
      glVertex2f(gui3D.window_width-2,  20);
      glVertex2f(gui3D.window_width-85, 20);
    }
    glEnd();
    glColor3f(0, 0, 0);
    snprintf(str, NAME_LENGTH, "%s",
        createTimeString( car_pose.timestamp+clock_offset_sec ) );
    week = (int) ((car_pose.timestamp - 315964800 ) / 604800);
    snprintf(str, NAME_LENGTH, "%s",
        createTimeString( car_pose.hardware_timestamp + 315964800 +
            week * 604800 ) );
    renderBitmapString(gui3D.window_width-80,7,
        GLUT_BITMAP_9_BY_15, str);
  }
