...
int main(int argc, char **argv)
{
  ...
  if(argc < 2)
    dgc_die("Error: Usage: %s program [arg1] [arg2] ...\n", argv[0]);
        
  spawned_name = argv[1];
  watchdog_pid = getpid();
 
  snprintf(ipc_name, 30, "watchdog_%s", argv[1]);
  ipc = new IpcStandardInterface;
  if( ipc->Connect(ipc_name) < 0)
    dgc_fatal_error("Could not connect to IPC network.");

  /* First argument will be "sh" plus one byte for the terminating null. */
  arg_run[0] = (char *)malloc((strlen("tcsh") + 1) * sizeof(char));
  dgc_test_alloc(arg_run[0]);
  ...
  while(1) {
    ...
    /* Notify the user that the child process needed to be restarted */
    if(WIFSIGNALED(status))
      snprintf(message_text, DGC_EVENT_NOTIFY_MESSAGE_LEN, 
          "Process %s exited due to an"
          "uncaught signal %d. Restarting.", argv[1], WSTOPSIG(status));
    if(WIFSTOPPED(status))
      snprintf(message_text, DGC_EVENT_NOTIFY_MESSAGE_LEN, 
          "Process %s was stopped. Restarting.", argv[1]);
    if(WIFEXITED(status))
      snprintf(message_text, DGC_EVENT_NOTIFY_MESSAGE_LEN, 
          "Process %s exited with status %d. Restarting.", 
          argv[1], WEXITSTATUS(status));
    
    SendEventNotification(ipc, "Watchdog", message_text);
