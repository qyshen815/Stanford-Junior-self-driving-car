...
int main(int /*argc*/, char **argv)
{
  ...
  PassatActuatorTorqueCommand(ipc, DGC_PASSAT_DIRECTION_FORWARD, 
			      0.0, commanded_brake, 0.0);
  fprintf(stderr, "\r      BRAKE COMMAND %d%%    ", 
          (int)rint(commanded_brake * 100.0));
  while(!quit_signal) {
    read_one = 0;
