               ...
               double origin_z, dgc_transform_t t, int color_mode, int flat)
{
  // t is the identity
  // char str[1000];
//   dgc_transform_print(velodyne_config->offset, str);
//   cout << str << endl;
  ...
  // Draw velo center.
  {
    ...
    dgc_transform_rpy(rotation, id,
		      vspin->scans[0].robot.roll,
		      vspin->scans[0].robot.pitch,
		      vspin->scans[0].robot.yaw);

    double x = velodyne_config->offset[0][3];
    double y = velodyne_config->offset[1][3];
    double z = velodyne_config->offset[2][3];
    dgc_transform_point(&x, &y, &z, rotation);

    x += vspin->scans[0].robot.x;
    y += vspin->scans[0].robot.y;
    z += vspin->scans[0].robot.z;
    
    cout << "Velo pose (blue): " << x << " " << y << " " << z << endl;
    glPushMatrix();
    glTranslatef(x - origin_x, y - origin_y, z - origin_z);
    glColor3f(0, 0, 1);
  ...
  glPushMatrix();
//   cout << "Robot pose (red): "
//        << vspin->scans[0].robot.x << " " 
//        << vspin->scans[0].robot.y << " "
//        << vspin->scans[0].robot.z << endl;
  glTranslatef(vspin->scans[0].robot.x - origin_x,
	       vspin->scans[0].robot.y - origin_y, 
	       vspin->scans[0].robot.z - origin_z);
  glColor3f(1, 0, 0);
  glutSolidSphere(.1, 10, 10);
  glPopMatrix();
  ...
  for(i = 0; i < vspin->num_scans; i++) 
    for(j = 0; j < VELO_BEAMS_IN_SCAN; j++) {
      ...
      octomap::OcTreeNode *node = g_map->search(x, y, z);
      if(!node) { 
	num_invalid++;
  ...
  cout << num_invalid << " invalid points." << endl;
}
...
int main(int argc, char **argv)
{
  ...
  /* Load map. */
  g_map = new octomap::OcTree(argv[2]);
  //g_map->setResolution(1);
  cout << "Map loaded, size " << g_map->size() << endl;
  ...
  //velodyne_offset is the same as velodyne_config->offset.
