		      ...
		      STRUCTMODEL *sm, int alg_type)
{
  ...
  if(sparm->slack_norm == 1) {
    ...
    lparm->sharedslack=0;
    if(kparm->kernel_type != SVMLIGHT_LINEAR_KERNAL_TYPE) {
	...
	for(i=0; i<n; i++) { /*** example loop ***/
	  ...
	  if((!use_shrinking) || (opti[i] != opti_round)) {
	    ...
	    if(slack > (margin-dist+0.0001)) {
	      ...
	      printf("Ex %d: functional margin=%f, slack=%f, newslack=%f\n",i,dist,slack,margin-dist);
	      /* exit(1); */
	    }
			    ...
			    STRUCTMODEL *sm, int alg_type)
{
  ...
  */
  ...
  for(i=0;i<n;i++) {
    if(USE_FYCACHE) {
      fy=psi(ex[i].x,ex[i].y,sm,sparm);
      if(kparm->kernel_type == SVMLIGHT_LINEAR_KERNAL_TYPE) { /* store difference vector directly */
	...
	run. */
{
  ...
  if(viol_gain > gainthresh) {
    fydelta_new=fydelta;
    if(struct_verbosity>=2) rt2=get_runtime();
    if(svmModel->kernel_parm.kernel_type == SVMLIGHT_LINEAR_KERNAL_TYPE) {
