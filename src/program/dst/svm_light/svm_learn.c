		     ...
		     optimization if not NULL. Array must be of size totdoc. */
{
  ...
  if(kernel_parm->kernel_type == SVMLIGHT_LINEAR_KERNAL_TYPE) {
    /* kernel_cache = NULL; */
  } 
  ...
  if(alpha) {
    ...
    if(kernel_cache && (kernel_parm->kernel_type != SVMLIGHT_LINEAR_KERNAL_TYPE)) {
     ...
     /* model:       Returns learning result (assumed empty before called) */
{
  ...
  /* caching makes no sense for linear kernel */
  if((kernel_parm->kernel_type == SVMLIGHT_LINEAR_KERNAL_TYPE) && (*kernel_cache)) {
     ...
     /* retrain: selects training mode (1=regular / 2=holdout) */
{
  ...
  for(;retrain && (!terminate);iteration++) {
	...
	if((minl(learn_parm->svm_newvarsinqp,
	   ...
	   && (kernel_parm->kernel_type != SVMLIGHT_LINEAR_KERNAL_TYPE)) {
    ...
    if((!retrain) && (inactivenum>0) 
	   ...
	   || (kernel_parm->kernel_type == SVMLIGHT_LINEAR_KERNAL_TYPE))) { 
      if(((verbosity>=1) && (kernel_parm->kernel_type != SVMLIGHT_LINEAR_KERNAL_TYPE)) 
	 || (verbosity>=2)) {
	if(verbosity==1) {
	  printf("\n");
	}
	...
	retrain=identify_one_misclassified(lin,label,unlabeled,totdoc,
      ...
      if(retrain) {
	if(kernel_parm->kernel_type == SVMLIGHT_LINEAR_KERNAL_TYPE) { /* reinit shrinking */
	  learn_parm->epsilon_crit=2.0;
	} 
      }
      if(verbosity>=1) {
     ...
     /* Check KT-conditions */
{
  ...
  if(kernel_parm->kernel_type == SVMLIGHT_LINEAR_KERNAL_TYPE) {  /* be optimistic */
        ...
        minshrink variables can be removed. */
{
  ...
  if((change>=minshrink) /* shrink only if sufficiently many candidates */
    ...
    if(kernel_parm->kernel_type != SVMLIGHT_LINEAR_KERNAL_TYPE) { /*  non-linear case save alphas */
      a_old=(double *)my_malloc(sizeof(double)*totdoc);
 	         ...
 	         values for linear kernel! */
{
  ...
  if(kernel_parm->kernel_type == SVMLIGHT_LINEAR_KERNAL_TYPE) { /* special linear case */
  ...
  if(kernel_parm->kernel_type != SVMLIGHT_LINEAR_KERNAL_TYPE) { /* update history for non-linear */
    for(i=0;i<totdoc;i++) {
