...
int main (int argc, char* argv[])
{
  ...
  while((!feof(docfl)) && fgets(line,(int)lld,docfl)) {
    ...
    totdoc++;
    if(model->kernel_parm.kernel_type == SVMLIGHT_LINEAR_KERNAL_TYPE) {/* For linear kernel,     */
    ...
    if(model->kernel_parm.kernel_type == SVMLIGHT_LINEAR_KERNAL_TYPE) {   /* linear kernel */
