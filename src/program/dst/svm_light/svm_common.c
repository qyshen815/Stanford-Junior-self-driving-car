...
// -- OpenCV defines MAX and MIN to be the same thing.
#ifndef MAX 
     ...
     /* classifies one example */
{
  ...
  if((model->kernel_parm.kernel_type == SVMLIGHT_LINEAR_KERNAL_TYPE) && (model->lin_weights))
     ...
     /* calculate the kernel function between two vectors */
{
  ...
  switch(kernel_parm->kernel_type) {
    case SVMLIGHT_LINEAR_KERNAL_TYPE: /* linear */ 
            return(sprod_ss(a,b)); 
     ...
     /* compute length of weight vector */
{
  ...
  if(model->kernel_parm.kernel_type != SVMLIGHT_LINEAR_KERNAL_TYPE) {
...
void write_model(char *modelfile, MODEL *model)
{
  ...
  if(0 && model->kernel_parm.kernel_type == SVMLIGHT_LINEAR_KERNAL_TYPE) {
    if(verbosity>=1) {
