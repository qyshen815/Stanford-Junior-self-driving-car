...
GLWidget::GLWidget(QWidget *parent)
     : QGLWidget(parent),
       window_width(width()), window_height(height()), fps(DEFAULT_FPS),
       user_display_func(NULL), user_keyboard_func(NULL), user_mouse_func(NULL), user_motion_func(NULL),
...
if (camera_pose.tilt < 0) {camera_pose.tilt = 0;}
else if (camera_pose.tilt > 89.0) {camera_pose.tilt = 89.0;}
}
 ...
 camera_pose.x_offset += float( dx * cos(rad(camera_pose.pan - 90.0)) *move_sensitivity
 		* camera_pose.distance);
 camera_pose.y_offset += float(dx * sin(rad(camera_pose.pan - 90.0)) *move_sensitivity
 		* camera_pose.distance);
 }

 void GLWidget::setCameraParams(double zoom_sensitivity_, double rotate_sensitivity_, double move_sensitivity_, double min_zoom_range_,
...
void GLWidget::setDisplayFunc(display_func func) {user_display_func = func;}

void GLWidget::setMouseFunc(mouse_func func) {user_mouse_func = func;}

void GLWidget::setMotionFunc(motion_func func) {user_motion_func = func;}
...
void GLWidget::activate3DMode(void)
{
...
gluLookAt(camera_x + camera_pose.x_offset, camera_y + camera_pose.y_offset, camera_z + camera_pose.z_offset, camera_pose.x_offset,
		camera_pose.y_offset, camera_pose.z_offset, 0, 0, 1);
}

void GLWidget::recenter(void)
{
camera_pose.x_offset = 0;
camera_pose.y_offset = 0;
camera_pose.z_offset = 0;
...
if (user_motion_func) {user_motion_func(int(event->x()), int(event->y()));}
last_mouse_x = event->x();
last_mouse_y = event->y();
