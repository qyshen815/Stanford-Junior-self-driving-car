...
/*class MultiTrackerTrack {
  public:
    MultiTrackerTrack( LinearKalmanFilter kf, std::tr1::shared_ptr<dgc::TrackedObstacle> obs);
    LinearKalmanFilter filter;
    std::tr1::shared_ptr<dgc::TrackedObstacle> obstacle;
};*/
...
class KalmanMultiTracker {
 private:
  ...
  void update(const std::vector< std::tr1::shared_ptr<dgc::Obstacle> >& measurements, double timestamp);

 public:
  std::list<std::tr1::shared_ptr<dgc::TrackedObstacle> >tracks_;
  ...
  void step(const std::vector<std::tr1::shared_ptr <dgc::Obstacle> >& measurements, double timestamp);
};
