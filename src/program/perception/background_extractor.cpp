...
void collectDataset() {
  ...
  for(int i = start_frame; i < end_frame; ++i) {
	...
	vector< boost::shared_ptr<Frame> > frames;
	dgc_pose_t pose; //TODO: Check these values.
	...
	pose.pitch = velodyne_index.spin[current_spin_num].pose[0].pitch;
	pose.yaw = velodyne_index.spin[current_spin_num].pose[0].yaw;
	boost::shared_ptr<Frame> frame(new Frame(dgcToRos(obstacles_segmented[j]->getPoints()),
						 velodyne_index.spin[current_spin_num].pose[0].timestamp,
						 pose, velodyne_offset));
	frames.push_back(frame);
	
	Track* tr = new Track("background", velodyne_offset, frames);
