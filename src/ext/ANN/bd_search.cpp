...
void ANNbd_shrink::ann_search(ANNSearchInputs *inputs, ANNmin_k *ANNkdPointMK,
			      ANNdist box_dist)
{
  register ANNpoint q = inputs->q;
	...
	if (inner_dist <= box_dist) {				// if inner box is closer
	  child[ANN_IN]->ann_search(inputs, ANNkdPointMK, inner_dist);	// search inner child first
	  child[ANN_OUT]->ann_search(inputs, ANNkdPointMK, box_dist);	// ...then outer child
	}
	else {										// if outer box is closer
	  child[ANN_OUT]->ann_search(inputs, ANNkdPointMK, box_dist);	// search outer child first
	  child[ANN_IN]->ann_search(inputs, ANNkdPointMK, inner_dist);	// ...then outer child
	}
