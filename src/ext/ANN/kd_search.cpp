...
void ANNkd_split::ann_search(ANNSearchInputs *inputs, ANNmin_k *ANNkdPointMK,
			     ANNdist box_dist)
{
  register double ANNkdMaxErr = inputs->ANNkdMaxErr;
  register ANNpoint q = inputs->q;
	...
	if (cut_diff < 0) {					// left of cutting plane
	  child[ANN_LO]->ann_search(inputs, ANNkdPointMK, box_dist);// visit closer child first
		...
		if (box_dist * ANNkdMaxErr < ANNkdPointMK->max_key())
		  child[ANN_HI]->ann_search(inputs, ANNkdPointMK, box_dist);

	}
	else {								// right of cutting plane
	  child[ANN_HI]->ann_search(inputs, ANNkdPointMK, box_dist);// visit closer child first
		...
		box_dist = (ANNdist) ANN_SUM(box_dist,
				ANN_DIFF(ANN_POW(box_diff), ANN_POW(cut_diff)));
										...
										// visit further child if close enough
		if (box_dist * ANNkdMaxErr < ANNkdPointMK->max_key())
		  child[ANN_LO]->ann_search(inputs, ANNkdPointMK, box_dist);
...
void ANNkd_leaf::ann_search(ANNSearchInputs *inputs, ANNmin_k *ANNkdPointMK,
			    ANNdist /*box_dist*/)
{
	...
	register ANNcoord t;
	register int d;
	register ANNpointArray ANNkdPts = inputs->ANNkdPts;
	register int ANNkdDim = inputs->ANNkdDim;
	register ANNpoint q = inputs->q;
