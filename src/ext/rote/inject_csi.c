...
static inline void clamp_cursor_to_bounds(RoteTerm *rt) {
   if (rt->crow < 0) rt->curpos_dirty = 1, rt->crow = 0;
   if (rt->ccol < 0) rt->curpos_dirty = 1, rt->ccol = 0;

   if (rt->crow >= rt->rows) 
      rt->curpos_dirty = 1, rt->crow = rt->rows - 1;

   if (rt->ccol >= rt->cols)
      rt->curpos_dirty = 1, rt->ccol = rt->cols - 1;
}
...
static void interpret_csi_SAVECUR(RoteTerm *rt, 
...
static void interpret_csi_RESTORECUR(RoteTerm *rt, 
				     int param[] __attribute__ ((unused)), 
				     int pcount __attribute__ ((unused))) {
